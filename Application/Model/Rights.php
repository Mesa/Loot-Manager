...
class Rights extends \JackAssPHP\Core\DataModel
{
    ...
    public function getUserRights ( $user_id )
    {
        if ( empty($this->userRightsStatement) ) {
            $this->userRightsStatement = $this->database->prepare("SELECT `RightId` FROM v_user_rights WHERE `v_user_rights`.`UserId` = :user_id;");
        }
    ...
    public function getGroupRights ( $group_id )
    {
        if ( empty($this->groupRightsStatement) ) {
            $this->groupRightsStatement = $this->database->prepare("SELECT `RightId` FROM v_group_rights WHERE `v_group_rights`.`GroupId` = :group_id;");
        }
    ...
    {
        $request = $this->getStatement("INSERT INTO `v_group_rights` (`GroupId`, `RightId`) VALUES (:group_id, (SELECT `Id` FROM `$this->DS_rights` WHERE `name` = :right_name));");
    ...
    {
        $request = $this->getStatement("DELETE FROM `v_group_rights` WHERE `GroupId` = :group_id and `RightId` = (SELECT `Id` FROM `$this->DS_rights` WHERE `name` = :right_name);");
        $request->bindParam(":group_id", $group_id);
        $request->bindParam(":right_name", $right_name);
        $request->execute();
    }
    ...
    {
        $request = $this->getStatement("INSERT INTO `v_user_rights` (`UserId`, `RightId`) VALUES (:user_id, (SELECT `Id` FROM `$this->DS_rights` WHERE `name` = :right_name));");
    ...
    {
        $request = $this->getStatement("DELETE FROM `v_user_rights` WHERE `UserId` = :user_id and `RightId` = (SELECT `Id` FROM `$this->DS_rights` WHERE `name` = :right_name);");
        $request->bindParam(":user_id", $user_id);
        $request->bindParam(":right_name", $right_name);
        $request->execute();
    }
