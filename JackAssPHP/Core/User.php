...
use JackAssPHP\Helper\Filter as Filter;
...
class User
{
    ...
    public function __construct ( $registry, $dao )
    {
        ...
        $this->dao = $dao;
        $this->login_redirect_path = $this->registry->get("WEB_ROOT")
                . $this->registry->get("LOGIN_REDIRECT_PATH");
        $this->getUserData();
    }
    ...
    public function login ( $username, $password, $remember_me = false )
    {
        ...
        $result = $this->dao->getUserByNamePassword(
            $this->encodeUsername($username), $this->hashPassword($password)
        );
    ...
    protected function setUserToken ()
    {
        ...
        setcookie(
            "rememberMeToken1", hash("sha256", $this->Name), time() + $this->cookie_expire, "/"
        );

        setcookie(
            "rememberMeToken2", hash("sha256", $_SERVER["REMOTE_ADDR"]), time() + $this->cookie_expire, "/"
        );

        setcookie(
            "rememberMeToken3", hash("sha256", time()), time() + $this->cookie_expire, "/"
        );

        setcookie(
            "rememberMeToken4", $token, time() + $this->cookie_expire, "/"
        );
